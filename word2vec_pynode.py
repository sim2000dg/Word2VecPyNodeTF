...
class algorithm_param:
    ...
    embedding_size = knext.IntParameter(label="Embedding size", default_value=20, min_value=1, description="Change the embedding size of the two Word2Vec embedding layers \
                                                                                                            (for target and context words, respectively) in order to get speed (smaller number) or \
                                                                                                            performance (larger number).")
                                        
    window_size = knext.IntParameter(label="Window size (radius)", min_value=1, default_value=2, description="Choose the radius of the window size that represents how far from the target \
    ...
    hierarchical_soft = knext.BoolParameter(label="Hierarchical Softmax", default_value=False, description="Activate hierarchical softmax in place of negative sampling. This option thus deactivates negative sampling.")                                                                                                                         
                                                                                                 
    algorithm_selection = knext.StringParameter(label="Word2Vec algorithm selection", description="Choose between CBOW (target as output) and skip-gram (context as output) Word2Vec implementation.", 
                                                enum = ["CBOW", "skip-gram"], default_value="skip-gram")
    

@knext.parameter_group(label="Dictionary Parameters")
class dictionary_param:
    
    word_survival_bool=knext.BoolParameter(label="Word Survival Function", default_value=True, description="Whether to use a word survival function \
                                                                                                            to reduce the size of the vocabulary by prioritizing rarer words.")
        
    word_survival_rate=knext.DoubleParameter(label="Sampling rate for Word Survival Function (if flagged)", max_value=0.1, min_value = 10**-10, default_value=10**-4,
    ...
    minimum_freq=knext.IntParameter(label="Minimum Frequency", min_value=0, default_value=5,
...
class training_param:
    ...
    nr_epoch = knext.IntParameter(label="Epochs", min_value=1, default_value = 10, description = "Number of epochs for model training. The more epoch, the longer time to train, linearly.")
    batch_size = knext.IntParameter(label="Batch size", min_value=8, default_value=32, description="The batch size you want to set to train the Word2Vec model.")
    adam_startinglr = knext.DoubleParameter(label="Adam learning rate", description="Set the learning rate for the Adam optimizer. The actual step in the parameter space is dynamic during training.", min_value=10**-10, default_value=10**-4)
...
@knext.node(name="Word2Vec Learner (Tensorflow)", node_type=knext.NodeType.LEARNER, icon_path="word2vec.png", category=python_category)
@knext.input_table(name="Table with string column", description="A KNIME table with a string column to use for Word2Vec training")
@knext.output_table(name="Table of embeddings", description="A KNIME table with three columns: the index of the token/the word, the token itself and the embedding for the token as a collection (KNIME native list).")
class Word2VecLearner:
    ...
    input_column = knext.ColumnParameter(label="Column selection (String type)", description="Select which document type column you want to use to train the model.", column_filter = is_string)
    
    word2vec_conf = Word2VecParam()
    training_conf = training_param()
    
    set_seed = knext.BoolParameter(label="Set seed", description="Set seeds for the whole node.", default_value=True)
    seed = knext.IntParameter(label="Seed", description="Choose the seed number, if you do not want the default one.", default_value=1234)
    ...
    def execute(self, exec_context, input_1):
        ...
        count_vectorizer_obj = sk_fe.text.CountVectorizer(lowercase=False, token_pattern=r"(?u)\b\w[-\w]+\b").fit(corpus)
        ...
        tf_table["keep_probability"] = (((tf_table["RF"]/self.word2vec_conf.group_2.word_survival_rate)**(1/2))+1)*(self.word2vec_conf.group_2.word_survival_rate/tf_table["RF"])
        tf_table["keep_probability"] = [1 if elem > 1 else elem for elem in tf_table["keep_probability"]]
        tf_table["label"] = random_gen.binomial(1, p=tf_table["keep_probability"]) # A binomial with n=1 is equal to a Bernoulli
        ...
        if not self.word2vec_conf.group_1.hierarchical_soft:
            ...
            for _ in range(len(target_context_skipgr)):
                negative_sampl.append(random_gen.choice(sampling_vector, size = self.word2vec_conf.group_1.negative_sample_int,
            ...
            if self.word2vec_conf.group_1.algorithm_selection == "skip-gram":
                training_input = tf.constant(target_context_skipgr[:,0], dtype = tf.int64)
                training_output = tf.constant(np.concatenate((np.expand_dims(target_context_skipgr[:, -1], axis=1), negative_sampl), axis = 1), dtype = tf.int64)
            else:
                training_input = tf.constant(context_target_CBOW[:, 0:-1], dtype=tf.int64)
                training_output = tf.constant(np.concatenate((np.expand_dims(context_target_CBOW[:, -1], axis=1), negative_sampl), axis = 1), dtype = tf.int64)
        ...
        if self.word2vec_conf.group_1.hierarchical_soft:
            frequency_tuples = [(token, freq) for token, freq in tf_table[["int_token", "TF"]].itertuples(index = False, name=None)]
        ...
        if self.word2vec_conf.group_1.algorithm_selection == "skip-gram":
            ...
            model = Word2Vec_CBOW_keras(vectorizer.vocabulary_size(), self.word2vec_conf.group_1.embedding_size, self.word2vec_conf.group_1.hierarchical_soft, self.word2vec_conf.group_1.window_size, self.word2vec_conf.group_1.negative_sample_int)
        ...
        model.compile(optimizer='adam', loss = custom_loss_word2vec(self.word2vec_conf.group_1.hierarchical_soft), metrics=[])
        with tf.device(self.tf_device):
            model.fit([training_input, training_output], training_label, batch_size=self.training_conf.batch_size, epochs=self.training_conf.nr_epoch, verbose=1, callbacks = CallbackforKNIME(exec_context, self.training_conf.nr_epoch))
            
        
        embeddings = model.get_layer("EmbeddingsWord2Vec").get_weights()[0][2:]
        embeddings = embeddings.tolist()
        embeddings_table = pd.DataFrame({"Token": tf_table["Token"].tolist(), 
                                         "Embedding": embeddings}, tf_table["int_token"], dtype=object)
